...
// هذا الملف جاهز للنشر الفوري كـ React App
export default function App() {
  ...
  const imsakiaData = useMemo(() => Array.from({ length: 30 }, (_, i) => ({
    day: ['السبت', 'الأحد', 'الاثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة'][i % 7],
    date: i + 1,
    fajr: '04:52', 
    imsak: '04:42',
    maghrib: '18:14'
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center ${isDarkMode ? 'bg-black text-white' : 'bg-stone-50 text-black'}`} dir="rtl">
      ...
      <nav className="flex gap-2 p-1 bg-white/5 rounded-2xl mb-8">
        {['player', 'readers', 'surahs', 'ramadan'].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === tab ? 'bg-emerald-500 text-white' : 'opacity-40'}`}>
            {tab === 'player' ? 'المشغل' : tab === 'readers' ? 'القراء' : tab === 'surahs' ? 'السور' : 'الإمساكية'}
          </button>
        ))}
      </nav>

      {/* Main Content */}
      <main className="flex-1 w-full max-w-4xl px-6 pb-20 overflow-y-auto no-scrollbar">
        {activeTab === 'player' && (
          <div className="flex flex-col items-center animate-in fade-in">
             <div className={`w-64 h-64 rounded-[3rem] bg-gradient-to-br ${readers[currentReaderIndex].color} flex items-center justify-center shadow-2xl relative transition-all ${isPlaying ? 'scale-105' : ''}`}>
             ...
             <div className="w-full mt-10">
                ...
                <div className="flex justify-between mt-4">
                  <button onClick={() => setCurrentSurahIndex(p => (p-1+surahs.length)%surahs.length)}><SkipBack/></button>
                  ...
                  <button onClick={() => setCurrentSurahIndex(p => (p+1)%surahs.length)}><SkipForward/></button>
                </div>
             </div>
          </div>
        )}
        ...
        {activeTab === 'readers' && (
          <div className="grid gap-3 animate-in slide-in-from-bottom">
            {readers.map((r, i) => (
        ...
        {activeTab === 'surahs' && (
          <div className="grid grid-cols-2 gap-3 animate-in slide-in-from-bottom">
            {surahs.map(s => (
        ...
        {activeTab === 'ramadan' && (
          <div className="bg-white/5 rounded-[2rem] overflow-hidden">
            <table className="w-full text-center text-sm">
              <thead className="bg-white/10">
                <tr><th className="p-3">رمضان</th><th className="p-3">اليوم</th><th className="p-3">فجر</th><th className="p-3">مغرب</th></tr>
              </thead>
              <tbody>
                {imsakiaData.map(d => (
                  <tr key={d.date} className="border-b border-white/5">
                    <td className="p-3 font-bold">{d.date}</td>
                    <td className="p-3 opacity-50">{d.day}</td>
                    <td className="p-3 text-emerald-400">{d.fajr}</td>
                    <td className="p-3 text-amber-400">{d.maghrib}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {showPrivacy && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-stone-900 p-8 rounded-[2rem] border border-white/10 max-w-sm">
            <h3 className="font-black text-blue-400 mb-4 flex items-center gap-2"><ShieldCheck size={20}/> سياسة الخصوصية</h3>
            <p className="text-sm opacity-70 leading-relaxed">هذا التطبيق لا يجمع أي بيانات، يحترم خصوصيتك بالكامل، ويعمل كصدقة جارية.</p>
            <button onClick={() => setShowPrivacy(false)} className="w-full mt-6 p-3 bg-white/5 rounded-xl font-bold">إغلاق</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-stone-900 p-8 rounded-[3rem] border border-white/10 w-full max-w-sm">
            ...
            <div className="space-y-4">
              <button onClick={() => setIsDarkMode(!isDarkMode)} className="w-full p-4 bg-white/5 rounded-2xl flex justify-between font-bold">
                <span>المظهر الليلي</span>
                {isDarkMode ? <Moon size={18}/> : <Sun size={18}/>}
              </button>
              <button onClick={() => {setShowPrivacy(true); setShowSettings(false)}} className="w-full p-4 bg-white/5 rounded-2xl flex justify-between font-bold">
